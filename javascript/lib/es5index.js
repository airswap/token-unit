...
if (typeof fs.readdirSync === "function") {
  // We are running on NodeJS
  tokens = _.values(require('require-all')({
    dirname: __dirname + '/' + unitDirectory
  }));
} else {
  // We may be on React
  tokens = [require(unitDirectory + '/ether.json'), require(unitDirectory + '/ast.json')];
}
...
var TokenUnit = function () {
  function TokenUnit(amount, unit) {
    var options = arguments.length <= 2 || arguments[2] === undefined ? {} : arguments[2];

    _classCallCheck(this, TokenUnit);
    ...
    this.anchorAmount = big(this.convertToAnchor(this.scaledAmount, this.scaleAndNominalUnitData));

    // Fill in the ether equivalent for this anchor amount
    this.etherEquivalent = big(this.findEtherEquivalent(this.anchorAmount, this.scaleAndNominalUnitData));
  ...
  */
  ...
  _createClass(TokenUnit, [{
    key: 'getScaleAndNominalUnit',
    value: function getScaleAndNominalUnit(unit) {
      if (!unit) {
    ...
    */
    ...
    key: 'convertToAnchor',
    value: function convertToAnchor(scaleAmount, scaleUnitData) {
    ...
    */
    ...
    key: 'findEtherEquivalent',
    value: function findEtherEquivalent(nominalAmount, nominalUnitData) {
    ...
    */
    ...
    key: 'anchorToEther',
    value: function anchorToEther(nominalUnit) {
      // TODO
    ...
    */
    ...
    value: function to(unit) {
      var toUnitData = this.getScaleAndNominalUnit(unit);

      if (this.scaleAndNominalUnitData.unit == toUnitData.unit) {
        // Just a scaling conversion
        return new TokenUnit(this.scaleFromNominal(this.anchorAmount, toUnitData.to_anchor), toUnitData.name);
      } else {
        // Currency conversion with a new nominal unit
        var marketQuoteFrom = this.anchorToEther(this.scaleAndNominalUnitData);
        var fromAmountToEther = big(this.anchorAmount).times(marketQuoteFrom.price);
        var marketQuoteTo = this.anchorToEther(toUnitData);
        var fromEtherToTokenAnchor = big(fromAmountToEther).div(marketQuoteTo.price);

        return new TokenUnit(this.scaleFromNominal(fromEtherToTokenAnchor, toUnitData.to_anchor), toUnitData.name, {
    ...
    */
    ...
    key: 'scaleFromNominal',
    value: function scaleFromNominal(nominalAmount, to_anchor) {
    ...
    */
    ...
    value: function add(tokenUnit) {
      ...
      return new TokenUnit(this.scaleFromNominal(this.anchorAmount.add(tokenUnit.anchorAmount), this.scaleAndNominalUnitData.to_anchor), this.scaledUnit);
    }
    ...
    */
    ...
    key: 'subtract',
    value: function subtract(tokenUnit) {
      if (!tokenUnit instanceof TokenUnit) {
        throw new Error("Argument passed in must be a TokenUnit type");
      }
      ...
      return new TokenUnit(this.scaleFromNominal(this.anchorAmount.sub(tokenUnit.anchorAmount), this.scaleAndNominalUnitData.to_anchor), this.scaledUnit);
    }
